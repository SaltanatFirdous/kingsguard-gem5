...
- `linux`: Linux kernel adapted for KingsGuard.
  Run:
  make ARCH=riscv CROSS_COMPILE=riscv64-unknown-linux-gnu- defconfig (in linux directory)
  make ARCH=riscv CROSS_COMPILE=riscv64-unknown-linux-gnu-  all -j16 (generates vmlinux)
- `riscv-pk`: Modified version of the RISC-V Proxy Kernel.
  ../configure --host=riscv64-unknown-linux-gnu --with-payload=<path to vmlinux> --prefix=<path to riscv cross compiler>  (in riscv-pk/build)
  make -j<n>

- `gem5`: Modified gem5 simulator used to model the Kingsguard architecture.
  Run:
  scons build/RISCV/gem5.opt -j<n>  (to build gem5 for RISCV)
  ./build/RISCV/gem5.opt configs/example/gem5_library/riscv-fs.py
  ./build/RISCV/gem5.opt configs/example/gem5_library/riscv-fs-sass.py (to run with SassCache implementation)
