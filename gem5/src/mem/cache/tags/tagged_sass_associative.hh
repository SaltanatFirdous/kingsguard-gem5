...
class TaggedSassAssociative : public TaggedIndexingPolicy
{
  protected:
    ...
    static constexpr int kCoverageT = -1; // start safe
    static constexpr int kRounds = 6;

    void initializeKeys();

    uint64_t hash_first_layer(Addr addr, unsigned way, uint64_t securityDomain) const;
    // uint64_t hash_second_layer(uint64_t scatterindex, unsigned way) const;
    uint64_t hash_second_layer(uint64_t scatterindex, unsigned way, uint64_t securityDomain) const;
